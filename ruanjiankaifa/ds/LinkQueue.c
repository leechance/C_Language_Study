...
/* 构造一个空队列Q */
Status lq_InitQueue(LinkQueue *Q)
{
...
/* 销毁队列Q */
Status lq_DestroyQueue(LinkQueue *Q)
{
    while(Q->front)
    {
        Q->rear=Q->front->next;
...
/* 将Q清为空队列 */
Status lq_ClearQueue(LinkQueue *Q)
{
    QueuePtr p,q;
    Q->rear=Q->front;
    p=Q->front->next;
...
Status lq_QueueEmpty(LinkQueue Q)
{
    if(Q.front==Q.rear)
        return TRUE;
    else
...
Status lq_GetHead(LinkQueue Q,QElemType *e)
{
    QueuePtr p;
    if(Q.front==Q.rear)
        return ERROR;
...
/* 从队头到队尾依次对队列Q中每个元素输出 */
Status lq_QueueTraverse(LinkQueue Q)
{
    QueuePtr p;
    p=Q.front->next;
    while(p)
